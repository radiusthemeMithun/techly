...
class Extras {
	...
	public function __construct() {
		...
		add_action( 'pre_get_posts', [ $this, 'techly_custom_pagesize' ],1 );
	...
	public function body_class( $classes ) {
		...
		$classes[] = 'techly-header-' . Opt::$header_style;
		$classes[] = 'techly-footer-' . Opt::$footer_style;
		...
		if ( techly_option( 'rt_tr_header_shadow' ) ) {
		...
		if ( techly_option( 'rt_sticy_header' ) ) {
			$classes[] = 'has-sticky-header';
		}

		if ( techly_option( 'rt_sticky_footer' ) ) {
			$classes[] = 'has-sticky-footer';
		}

		if ( techly_option( 'rt_blend' ) ) {
			$classes[] = 'rt-blend';
		}

		if ( techly_option( 'rt_remove_admin_bar' ) ) {
			$classes[] = 'remove-admin-bar';
		}

		if ( is_single() && Opt::$single_style ) {
			$classes[] = 'techly-single-' . Opt::$single_style;
		}
	...
	/*custom team archive */
	function techly_custom_pagesize( $query ) {
	...
	function menu_customize( $item_id, $item ) {
		// Mega menu
		$_mega_menu = get_post_meta( $item_id, 'techly_mega_menu', true );
		// Query string
		$menu_query_string = get_post_meta( $item_id, 'techly_menu_qs', true );
		?>
	...
	function menu_modify( $items, $menu, $args ) {
		foreach ( $items as $item ) {
			$menu_query_string = get_post_meta( $item->ID, 'techly_menu_qs', true );
	...
	public function search_form() {
		$output = '
		<form method="get" class="techly-search-form" action="' . esc_url( home_url( '/' ) ) . '">
            <div class="search-box">
				<input type="text" class="form-control" placeholder="' . esc_attr__( 'Search here...', 'techly' ) . '" value="' . get_search_query() . '" name="s" />
				<button class="item-btn" type="submit">
					' . techly_get_svg( 'search', false ) . '
					<span class="btn-label">' . esc_html__( 'Search', 'techly' ) . '</span>
				</button>
            </div>
		</form>
		';
