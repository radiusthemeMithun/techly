...
class DynamicStyles {
	...
	public function enqueue_scripts() {
		...
		wp_register_style( 'techly-dynamic', false, 'techly-main' );
		wp_enqueue_style( 'techly-dynamic' );
		wp_add_inline_style( 'techly-dynamic', $this->minify_css( $dynamic_css ) );
	}
	...
	private function inline_style() {
		...
		$primary_color   	= techly_option( 'rt_primary_color', '#0F53F3' );
		$secondary_color 	= techly_option( 'rt_secondary_color', '#F2BF18' );
		$tertiary_color    	= techly_option( 'rt_tertiary_color', '#ffb000' );
		$body_bg_color   	= techly_option( 'rt_body_bg_color', '#ffffff' );
		$body_color      	= techly_option( 'rt_body_color', '#484848' );
		$border_color      	= techly_option( 'rt_border_color', '#D9D9D9' );
		$title_color     	= techly_option( 'rt_title_color', '#000000' );
		$button_color    	= techly_option( 'rt_button_color', '#0F53F3' );
		$button_text_color 	= techly_option( 'rt_button_text_color', '#ffffff' );
		$black_color 		= techly_option( 'rt_button_text_color', '#ffffff' );
		$meta_color      	= techly_option( 'rt_meta_color', '#D5ED9E' );
		$meta_light      	= techly_option( 'rt_meta_light', '#F3F7FF' );
		$gray10          	= techly_option( 'rt_gray10_color', '#F3F7FF' );
		$gray20          	= techly_option( 'rt_gray20_color', '#E1EDE6' );
	...
	protected function topbar_css() {
		...
		if ( ! empty( $_topbar_active_color ) ) : ?>
			...
			body .techly-topbar #topbar-menu ul ul li.current_page_item > a,
			body .techly-topbar #topbar-menu ul ul li.current-menu-ancestor > a,
			body .techly-topbar #topbar-menu ul.techly-topbar-menu > li.current-menu-item > a,
			body .techly-topbar #topbar-menu ul.techly-topbar-menu > li.current-menu-ancestor > a {
			color: <?php echo esc_attr( $_topbar_active_color ); ?>;
			}

			body .site-header .techly-topbar .topbar-container a:hover:not(.dropdown-menu a:hover svg) svg,
			body .techly-topbar #topbar-menu ul ul li.current-menu-ancestor > a svg,
			body .techly-topbar #topbar-menu ul.techly-topbar-menu > li.current-menu-item > a svg,
			body .techly-topbar #topbar-menu ul.techly-topbar-menu > li.current-menu-ancestor > a svg {
	...
	protected function header_css() {
		...
		$logo_dimension     = techly_option( 'rt_logo_width_height' );
		$logo_mobile_dimension     = techly_option( 'rt_mobile_logo_width_height' );
		$menu_border_bottom = techly_option( 'rt_menu_border_color' );
		...
		//Default Menu
		$_menu_color        = techly_option( 'rt_menu_color' );
		$_menu_active_color = techly_option( 'rt_menu_active_color' );
		$_menu_bg_color     = techly_option( 'rt_menu_bg_color' );
		$_sub_menu_bg_color     = techly_option( 'rt_sub_menu_bg_color' );

		//Transparent Menu
		$_tr_menu_color        = techly_option( 'rt_tr_menu_color' );
		$_tr_menu_active_color = techly_option( 'rt_tr_menu_active_color' );

		$_header_border     = techly_option( 'rt_header_border' );
		$_breadcrumb_border = techly_option( 'rt_breadcrumb_border' );
		$_preloader_bg_color = techly_option( 'preloader_bg_color' );
		?>

		<?php //Header Logo CSS ?>
		<?php if ( Opt::$header_width == 'full' ) :
			$h_width = '100%';
			if ( ( $header_width = techly_option( 'rt_header_max_width' ) ) > 768 ) {
		...
		<?php if ( ! empty( $_menu_color ) ) : ?>
			body .techly-navigation ul li a,
			body .techly-offcanvas-drawer ul.menu li a,
			body .techly-navigation ul li ul li a,
			body .menu-icon-wrapper .menu-search-bar {
				color: <?php echo esc_attr( $_menu_color ) ?>;
			}
			body .main-header-section svg,
			body .techly-navigation .caret svg {
		...
		<?php if ( ! empty( $_menu_active_color ) ) : ?>
			body .techly-navigation ul li a:hover,
			body .techly-navigation ul li.current-menu-item > a,
			body .techly-navigation ul li.current-menu-ancestor > a,
			body .techly-offcanvas-drawer ul li.current-menu-ancestor > a,
			body .techly-offcanvas-drawer ul.menu li a:hover,
			body .techly-navigation ul li ul li a:hover {
				color: <?php echo esc_attr( $_menu_active_color ) ?>;
			}
			body .techly-navigation ul li a:hover svg,
			body .techly-navigation ul li.current-menu-item > a svg,
			body .techly-navigation ul li.current-menu-ancestor > a svg {
		...
		<?php if ( ! empty( $_sub_menu_bg_color ) ) : ?>
			body .techly-navigation ul > li > ul,
			body .techly-navigation ul li.mega-menu > ul.dropdown-menu{
		...
		<?php if ( ! empty( $_tr_menu_color ) ) : ?>
			...
			body.has-trheader .site-header .techly-navigation *,
			body.has-trheader .site-header .techly-navigation ul li a {
			...
			body.has-trheader .site-header .techly-topbar .caret svg,
		...
		<?php if ( ! empty( $_tr_menu_active_color ) ) : ?>
			body.has-trheader .site-header .techly-navigation ul li a:hover,
			body.has-trheader .site-header .techly-navigation ul li.current-menu-item > a,
			body.has-trheader .site-header .techly-navigation ul li.current-menu-ancestor > a {
			...
			body.has-trheader .site-header .techly-navigation ul li.current-menu-ancestor > a svg,
			body.has-trheader .site-header .techly-navigation ul li.current-menu-item > a svg {
		...
		<?php if ( ! empty( $menu_border_bottom ) ) : ?>
			body .techly-topbar,
			body .main-header-section,
			body .techly-breadcrumb-wrapper {
		...
		<?php if ( ! $_breadcrumb_border ) : ?>
			body .techly-breadcrumb-wrapper {border-bottom: none;}
		<?php endif; ?>
	...
	protected function breadcrumb_css() {
		$breadcrumb_color          = techly_option( 'rt_breadcrumb_color' );
		$rt_breadcrumb_hover       = techly_option( 'rt_breadcrumb_hover' );
		$breadcrumb_active         = techly_option( 'rt_breadcrumb_active' );
		$rt_breadcrumb_title_color = techly_option( 'rt_breadcrumb_title_color' );
		$rt_banner_color_opacity = techly_option( 'rt_banner_color_opacity' );

		$rt_banner_padding_top = techly_option( 'rt_banner_padding_top' );
		$rt_banner_padding_bottom = techly_option( 'rt_banner_padding_bottom' );

		$rt_banner_overlay1_color = techly_option( 'rt_banner_overlay1_color' );
		$rt_banner_overlay2_color = techly_option( 'rt_banner_overlay2_color' );
		...
		if ( ! empty( $rt_banner_overlay1_color && $rt_banner_overlay2_color ) ) { ?>
			.techly-breadcrumb-wrapper.has-bg::before {
	...
	protected function content_padding_css() {
		...
		static function css( $selector, $property, $theme_mod, $after_css = '' ) {
			$theme_mod = techly_option( $theme_mod );
