...
use RT\Techly\Modules\PostShare;

/*Allow HTML for the kses post*/
function techly_html( $html, $context = '' ) {
...
if ( ! function_exists( 'techly_custom_menu_cb' ) ) {
	/**
...
if ( ! function_exists( 'techly_menu_icons_group' ) ) {
	/**
	 * Get menu icon group
	 * @return void
	 */
	function techly_menu_icons_group( $args = [] ) {
		$default_args = [
			'hamburg'       => techly_option( 'rt_header_bar' ),
			'search'        => techly_option( 'rt_header_search' ),
			'login'         => techly_option( 'rt_header_login_link' ),
			'button'        => techly_option( 'rt_get_started_button' ),
			'button_label'  => techly_option( 'rt_get_started_label' ),
			'button_link'   => techly_option( 'rt_get_started_button_url' ),
			'has_separator' => techly_option( 'rt_header_separator' ),
			'compare'    	=> techly_option( 'rt_header_compare' ),
			'wishlist'    	=> techly_option( 'rt_header_wishlist' ),
			'add_to_cart' 	=> techly_option( 'rt_header_add_to_cart' )
		];
		...
		<div class="menu-icon-wrapper ml-auto">
			<ul class="menu-icon-action <?php echo esc_attr( $menu_classes ) ?>">
				<?php if ( $args['hamburg'] ) : ?>
					<?php techly_hanburger( 'desktop-hamburg' ); ?>
				<?php endif; ?>

				<?php techly_hanburger( 'mobile-hamburg' ); ?>
				...
				<?php if ( $has_button ) : ?>
					<li class="rt-button">
						<a class="btn button-4" href="<?php echo esc_url( $args['button_link'] ) ?>" aria-label="button link">
							<span class="button-text"><?php echo esc_html( $args['button_label'] ); ?></span>
						</a>
					</li>
				<?php endif; ?>
			</ul>
...
if ( ! function_exists( 'techly_require' ) ) {
	/**
	 ...
	 * @return false|void
	 */
	function techly_require( $filename, string $dir = 'inc' ) {
...
if ( ! function_exists( 'techly_readmore_text' ) ) {
	/**
	 * Creates continue reading text.
	 *
	 * @return string
	 */
	function techly_readmore_text() {

		if ( empty( techly_option( 'rt_blog_read_more' ) ) ) {
			return;
		}
		return sprintf(
			'%s %s',
			esc_html( techly_option( 'rt_blog_read_more' ) ),
			the_title( '<span class="screen-reader-text">', '</span>', false )
		);
	}
}

if ( ! function_exists( 'techly_list_item_separator' ) ) {
	/**
	 ...
	 * @since 6.0.0
	 */
	function techly_list_item_separator() {
...
if ( ! function_exists( 'techly_posted_in' ) ) {
	/**
	 * Prints HTML with category list information about theme categories.
	 * @return string
	 */
	function techly_posted_in( $type = 'category' ) {
		$categories_list = get_the_category_list( techly_list_item_separator() );
		if ( 'tag' === $type ) {
			$categories_list = get_the_tag_list( '', techly_list_item_separator() );
		}
		if ( $categories_list ) {
			return sprintf(
...
if ( ! function_exists( 'techly_posted_on' ) ) {
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 * @return string
	 */
	function techly_posted_on() {
		$time_string = sprintf(
...
if ( ! function_exists( 'techly_posted_by' ) ) {
	/**
...
if ( ! function_exists( 'techly_get_svg' ) ) {
	/**
	 * Get svg icon
	 *
	 * @param $name
	 ...
	 * @return string|void
	 */
	function techly_get_svg( $name, $rotate = '' ) {
		$svg_list     = apply_filters( 'techly_svg_icon_list', [
...
if ( ! function_exists( 'techly_get_file' ) ) {
	/**
	 * Get File Path
	 *
	 * @param $path
	 ...
	 * @return string
	 */
	function techly_get_file( $path, $return_path = false ): string {
...
if ( ! function_exists( 'techly_get_img' ) ) {
	/**
	 * Get Image Path
	 *
	 * @param $filename
	 ...
	 * @return string|void
	 */
	function techly_get_img( $filename, $echo = false, $image_meta = '' ) {
		$path      = '/assets/images/' . $filename;
		$image_url = techly_get_file( $path );
...
if ( ! function_exists( 'techly_get_css' ) ) {
	/**
	 * Get CSS Path
	 *
	 * @param $filename
	 ...
	 * @return string
	 */
	function techly_get_css( $filename, $check_rtl = false ) {
		$min    = WP_DEBUG ? '.css' : '.css';
		$is_rtl = $check_rtl && is_rtl() ? 'css-rtl' : 'css';
		$path   = "/assets/$is_rtl/" . $filename . $min;

		return techly_get_file( $path );
	}
}



if ( ! function_exists( 'techly_get_js' ) ) {
	/**
	 * Get JS Path
	 *
	 * @param $filename
	 ...
	 * @return string
	 */
	function techly_get_js( $filename ) {
...
if ( ! function_exists( 'techly_option' ) ) {
	/**
	 ...
	 * @return mixed
	 */
	function techly_option( $key, $default = '', $return_array = false ) {
...
if ( ! function_exists( 'techly_get_social_html' ) ) {
	/**
	 * Get Social markup
	 *
	 * @param $color
	...
	function techly_get_social_html( $color = '' ) {
		ob_start();
...
if ( ! function_exists( 'techly_site_logo' ) ) {
	/**
	 * Newfit Site Logo
	 *
	 */
	function techly_site_logo( $with_logo = false, $custom_title = '', $mode = '' ) {
		$logo_main       = techly_option( 'rt_logo' );
		$logo_light      = techly_option( 'rt_logo_light' );
		$logo_mobile     = techly_option( 'rt_logo_mobile' );
...
if ( ! function_exists( 'techly_footer_logo' ) ) {
	/**
	 * Newfit Site Logo
	 *
	 */
	function techly_footer_logo() {
		$main_logo  = techly_option( 'rt_logo' );
		$logo_light = techly_option( 'rt_logo_light' );
		$site_logo  = $main_logo;
...
if ( ! function_exists( 'techly_scroll_top' ) ) {
	/**
	 * Back-to-top button
	 * @return void
	 */
	function techly_scroll_top( $class = '', $icon = 'scroll-top' ) {
		if ( techly_option( 'rt_back_to_top' ) ) {
			?>
			<a href="#" class="scrollToTop <?php echo esc_attr( $class ) ?>">
				<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path d="M5 8C5.742 8 6.85 7.267 7.78 6.525C8.98 5.571 10.027 4.431 10.826 3.124C11.425 2.144 12 0.956 12 0M12 0C12 0.956 12.575 2.145 13.174 3.124C13.974 4.431 15.021 5.571 16.219 6.525C17.15 7.267 18.26 8 19 8M12 0L12 24" stroke="currentColor" stroke-width="1.5"/>
				</svg>
			</a>
			<?php
		}
...
if ( ! function_exists( 'techly_meta_icons' ) ) {
	/**
	 * Get meta icons
	 *
	 * @param $name
	 ...
	 * @return string|void
	 */
	function techly_meta_icons( $name ) {
		if ( ! $name ) {
			return;
		}
		$icon_list = [
			...
			'comment'  => '<i class="icon-rt-comment"></i>',
			'reading'  => '<i class="icon-rt-clock"></i>',
			'views'    => '<i class="icon-eye"></i>',
			'category' => '<i class="icon-rt-gift"></i>',
...
if ( ! function_exists( 'techly_reading_time' ) ) {
	/**
	 * Post reading time
	 */
...
if( !function_exists( 'rt_post_views' )){
	...
	function rt_post_views( $text = '', $post_id = 0 ){
		...
		if ( $view_count == 1 ) {
			$techly_view_html = esc_html__( 'View' , 'techly' );
		} else {
			$techly_view_html = esc_html__( 'Views' , 'techly' );
		}

		$techly_views_html = '<span class="view-number" >' . $view_count . '</span> ' . $techly_view_html;

		return '<span class="meta-views meta-item '. $views_class .'">'. $techly_views_html.'</span> ';
	}
}

if ( ! function_exists( 'techly_post_meta' ) ) {
	/**
	 * Get post meta
	 *
	 * @return string
	 */
	function techly_post_meta( $args ) {
		$default_args = [
			'with_list'     => true,
			'with_icon'     => true,
			'include'       => [],
			'class'         => '',
			'author_prefix' => __( 'By', 'techly' )
		];
		...
		$_meta_data['author']   = techly_posted_by( $args['author_prefix'] );
		$_meta_data['date']     = techly_posted_on();
		$_meta_data['category'] = techly_posted_in();
		$_meta_data['tag']      = techly_posted_in( 'tag' );
		$_meta_data['comment']  = esc_html( $comments_text );
		$_meta_data['reading']  = techly_reading_time();
		...
		foreach ( $meta_list as $key ) {
			...
			$output .= $args['with_icon'] ? techly_meta_icons( $key ) : null;
			$output .= $meta;
...
if ( ! function_exists( 'techly_post_thumbnail' ) ) {
	/**
	 * Displays post thumbnail.
	 * @return void
	 */
	function techly_post_thumbnail( $size = 'full', $thumb_date = false ) {
...
if ( ! function_exists( 'techly_post_single_thumbnail' ) ) {
	/**
	 * Display post details thumbnail
	 * @return void
	 */
	function techly_post_single_thumbnail( $size = 'full' ) {
		if ( ! Fns::can_show_post_thumbnail() ) {
			return;
		}
		?>
...
if ( ! function_exists( 'techly_entry_footer' ) ) {
	/**
	 * Techly Entry Footer
	 *
	 * @return void
	 *
	 */
	function techly_entry_footer() {
		if ( ! is_single() ) {
			if ( techly_option( 'rt_blog_footer_visibility' ) ) { ?>
				<footer class="entry-footer rt-button">
					<a class="btn <?php echo esc_attr( techly_option('rt_blog_btn_style') ); ?>"
					   href="<?php echo esc_url( get_permalink() ) ?>" <?php if( is_numeric(techly_option('rt_blog_btn_radius') ) ) { ?>
						style="border-radius: <?php echo esc_attr( techly_option('rt_blog_btn_radius') ); ?>px"<?php } ?>>
			...
			if ( ( has_tag() && techly_option( 'rt_single_tag_visibility' ) ) || techly_option( 'rt_single_share_visibility' ) ) { ?>
				<footer class="entry-footer d-flex align-items-center justify-content-between">
					<?php if ( techly_option( 'rt_single_tag_visibility' ) ) { ?>
						<div class="post-tags">
							<?php if ( $tags_label = techly_option( 'rt_tags' ) ) {
								printf( "<span>%s</span>", esc_html( $tags_label ) );
							} ?>

							<?php
							techly_single_post_footer_meta(
								'content-below-meta', [ 'tag' ]
							);
							?>
						</div>
					<?php }
					if ( techly_option( 'rt_single_share_visibility' ) ) { ?>
						<div class="post-share">
							<?php if ( $tags_label = techly_option( 'rt_social' ) ) {
								printf( "<span>%s</span>", esc_html( $tags_label ) );
							} ?>
							<a href="#" class="share-icon"><i class="icon-rt-share"></i></a>
							<?php PostShare::techly_post_share(); ?>
						</div>
					<?php } ?>
				</footer>
				<?php
...
// single video
if ( ! function_exists( 'techly_post_single_video' ) ) {
	/**
	 * Display post details video
	 * @return void
	 */
	function techly_post_single_video() { ?>
		<?php $rt_youtube_link = get_post_meta( get_the_ID(), 'rt_youtube_link', true );
...
if ( ! function_exists( 'techly_entry_profile' ) ) {
	/**
	 * Techly Entry Profile
	 *
	 * @return void
	 *
	 */
	function techly_entry_profile() {
		if ( techly_option( 'rt_single_profile_visibility' ) ) {
					...
					<div class="profile-content">
						<div class="profile-author-info">
							...
							<div class="profile-info">
								...
								<?php if ( $prof_phone ) { ?>
									<span class="profile-phone"><?php echo esc_html__( 'Ph: ', 'techly' ) ?><?php echo esc_html( $prof_phone );?></span>
								<?php } ?>
							</div>
						</div>
...
if ( ! function_exists( 'techly_single_post_footer_meta' ) ) {
	/**
...
if ( ! function_exists( 'techly_entry_content' ) ) {
	/**
	 * Entry Content
	 * @return void
	 */
...
if ( ! function_exists( 'techly_sidebar' ) ) {
	/**
	 ...
	 * @return false|void
	 */
	function techly_sidebar( $sidebar_id ) {
...
if ( ! function_exists( 'techly_post_class' ) ) {
	/**
	 * Get dynamic article classes
	 * @return string
	 */
	function techly_post_class( $default_class = 'techly-post-card rt-grid-item' ) {
		$above_meta_style = 'above-' . techly_option( 'rt_single_above_meta_style' );


		$common_clsss = '';
		if ( is_single() ) {
			$common_clsss .= techly_option( 'rt_single_above_meta_visibility' ) ? 'is-above-meta' : 'no-above-meta';
			$meta_style   = techly_option( 'rt_single_meta_style' );
			$post_classes = Fns::class_list( [ $common_clsss, $meta_style, $above_meta_style ] );
		} else {
			$common_clsss .= techly_option( 'rt_blog_above_meta_visibility' ) ? 'is-above-meta' : 'no-above-meta';
			$meta_style   = techly_option( 'rt_blog_meta_style' );
			$blog_style   = 'blog-' . techly_option( 'rt_blog_style' );
...
if ( ! function_exists( 'techly_separate_meta' ) ) {
	/**
...
if ( ! function_exists( 'techly_single_entry_header' ) ) {
	/**
	 * Get above title meta
	 * @return string
	 */
	function techly_single_entry_header() {
		?>
...
if ( ! function_exists( 'techly_breadcrumb' ) ) {
	/**
	 * Techly breadcrumb
	 * @return void
	 */
	function techly_breadcrumb() {
		?>
		<nav aria-label="breadcrumb">
			<ul class="breadcrumb">
				<li class="breadcrumb-item">
					<a href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'Home', 'techly' ) ?></a>
					<span class="raquo"><i class="icon-rt-user-datalist-feature"></i></span>
				</li>
				<li class="breadcrumb-item active" aria-current="page">
					<?php
					if ( is_tag() ) {
						esc_html_e( 'Posts Tagged: ', 'techly' );
						?>
						...
						esc_html_e( 'Posts made in: ', 'techly' );
						echo esc_html( get_the_time( is_year() ? 'Y' : ( is_month() ? 'F, Y' : 'F jS, Y' ) ) );
						echo '</span>';
					} elseif ( is_search() ) {
						echo '<span class="title">';
						esc_html_e( 'Search results for: ', 'techly' );
						the_search_query();
						echo '</span>';
					} elseif ( is_404() ) {
						echo '<span class="title">';
						esc_html_e( '404', 'techly' );
						echo '</span>';
					} elseif ( is_single() ) {
...
if ( ! function_exists( 'techly_get_avatar_url' ) ) :
	function techly_get_avatar_url( $get_avatar ) {
...
function techly_comments_cbf( $comment, $args, $depth ) {
	...
	switch ( $comment->comment_type ) :
		...
		case 'trackback' : ?>
			<div class="pingback-entry"><span
					class="pingback-heading"><?php esc_html_e( 'Pingback:', 'techly' ); ?></span> <?php comment_author_link(); ?>
			</div>
			<?php
			break;
		default :
...
if ( ! function_exists( 'techly_hanburger' ) ) {
	 ...
	 * @return void
	 */
	function techly_hanburger( $class = '' ) {
		?>
		<li class="ham-burger <?php echo esc_attr( $class ) ?>">
			<button type="button" class="menu-bar trigger-off-canvas" aria-label="hamburger menu">
				<?php if( techly_option('rt_get_menu_label') ) { ?>
					<span class="menu-label"><?php echo esc_html( techly_option('rt_get_menu_label') ) ?></span>
				<?php } ?>
				<span class="btn-hamburger">
					...
					<svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
						...
						<rect y="22" width="6" height="6" rx="2" fill="currentColor"/>
						<rect x="11" width="6" height="6" rx="2" fill="currentColor"/>
						<rect x="11" y="11" width="6" height="6" rx="2" fill="currentColor"/>
						<rect x="11" y="22" width="6" height="6" rx="2" fill="currentColor"/>
						<rect x="22" width="6" height="6" rx="2" fill="currentColor"/>
						<rect x="22" y="11" width="6" height="6" rx="2" fill="currentColor"/>
						<rect x="22" y="22" width="6" height="6" rx="2" fill="currentColor"/>
					</svg>
				</span>
			</button>
		</li>
