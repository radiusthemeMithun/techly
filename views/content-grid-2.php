...
$meta_list = techly_option( 'rt_blog_meta', '', true );
if ( techly_option( 'rt_blog_above_meta_visibility' ) && !empty( has_post_thumbnail() ) && !empty( $rt_post_gallerys_raw ) ) {
	$meta_index = array_search( 'date', $meta_list );
	unset( $meta_list[ $meta_index ] );
}

$wow = techly_option('rt_animation') == 1 ? 'wow' : 'animation-off';
$effect = techly_option('rt_animation_effect');
$delay = techly_option('delay');
$duration = techly_option('duration');
?>
<article data-post-id="<?php the_ID(); ?>" <?php post_class( techly_post_class() ); ?>>
	<div class="article-inner-wrapper <?php echo esc_attr($wow . ' ' . $effect); ?>" data-wow-delay="<?php echo esc_attr($delay); ?>ms" data-wow-duration="<?php echo esc_attr($duration); ?>ms">
		<div class="post-thumbnail-wrap">
					...
					<div class="swiper-wrapper">
						<?php foreach( $rt_post_gallerys as $rt_posts_gallery ) { ?>
							<div class="swiper-slide">
								<?php echo wp_get_attachment_image( $rt_posts_gallery, 'techly-size3', "", array( "class" => "img-responsive" ) );
								?>
							</div>
						<?php } ?>
					</div>
			...
			<?php } else { ?>
				<figure class="post-thumbnail">
					<a class="post-thumb-link alignwide" href="<?php the_permalink(); ?>" aria-hidden="true" tabindex="-1"><?php the_post_thumbnail( 'medium_large' ); ?></a>
				</figure><!-- .post-thumbnail -->
				<?php if ( techly_option( 'rt_blog_above_meta_visibility' ) && !empty( has_post_thumbnail() )) { ?>
		...
		<div class="entry-wrapper">
			...
			<?php if ( techly_option( 'rt_blog_content_visibility' ) ) : ?>
