...
if ( techly_option( 'rt_project_style' ) == 'default' ){
	$project_archive_layout 		= "rt-project-default rt-project-multi-layout-1";
} elseif ( techly_option( 'rt_project_style' ) == '2' ){
						...
						<div class="row g-4 item-parent">
							<?php while ( have_posts() ) : the_post(); ?>
								<div class="<?php echo esc_attr( $post_classes );?> item">
									<?php get_template_part( 'views/content-project', techly_option( 'rt_project_style' ) ); ?>
								</div>
							<?php endwhile; ?>
						</div>
					<?php else:?>
